...
public class Shows {
    ...
    public static void swap(ArrayList<Show> list, int i, int j) {
        Show tmp = list.get(i);
        list.set(i, list.get(j));
        list.set(j, tmp);
    }

    //Quicksort function to sort the shows by title
    public static void quicksort(ArrayList<Show> list, int left, int right) {
        int i = left, j = right;
        String pivot = list.get((right + left) / 2).getTitle();
    ...
    public static void main(String[] args) {
        ...
        quicksort(userShows, 0, userShows.size() - 1);
