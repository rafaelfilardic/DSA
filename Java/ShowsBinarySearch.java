...
public class Shows {
    public static boolean binarySearch(ArrayList<Show> list, String showId) {
        int left = 0;
        int right = list.size() - 1;

        while (left <= right) {
            int mid = left + (right - left) / 2;
            int comparison = list.get(mid).getShowId().compareTo(showId);
    ...
    public static void main(String[] args) {
        ...
        //Sorts shows list
        userShows.sort(Comparator.comparing(Show::getShowId));

        while(!(input = sc.nextLine()).equals("FIM")) {
            try {
                String showIdToSearch = input;
                if(binarySearch(userShows, showIdToSearch)) {
