...
class BinarySearchTree {
    ...
    private Node insert(Node current, Show show) {
        if(current == null) {
            return new Node(show);
        }

        //Compare titles to check where to insert
        if(show.getTitle().compareTo(current.show.getTitle()) < 0) {
            current.left = insert(current.left, show);
        }
        else if(show.getTitle().compareTo(current.show.getTitle()) > 0) {
            current.right = insert(current.right, show);
        }
        else {
            return current;
        }
    ...
    private void preOrderTraversal(Node current) {
        if(current != null) {
            current.show.print(current.show);
...
public class Shows {
    public static void main(String[] args) {
        ...
        try(BufferedReader br = new BufferedReader(new FileReader(filePath))) {
            ...
            return; //Exits the program if file canNodet be read
        }

        Scanner sc = new Scanner(System.in);
        BinarySearchTree userShows = new BinarySearchTree();
        ...
        while(!(input = sc.nextLine()).equals("FIM")) {
            try {
                ...
                if(showId > 0 && showId <= shows.size()) {
                    userShows.insert(shows.get(showId - 1));
                }
                else {
